...
if (!function_exists('stm_importer_done_function')) {
    function stm_importer_done_function()
    {
        ...
        if($chosen_template == 'car_dealer' || $chosen_template == 'boats' || $chosen_template == 'motorcycle'){
        ...
        if ($chosen_template == 'listing') {
			...
			$termmeta = json_decode(file_get_contents(get_template_directory() . '/inc/demo/model_json.json'));
        ...
        if ($chosen_template == 'car_rental') {
            /*Widgets*/
            $widgets_file = get_template_directory() . '/inc/demo/widget_data_rental.json';
            if (file_exists($widgets_file)) {
                $encode_widgets_array = $wp_filesystem->get_contents($widgets_file);
                stm_import_widgets($encode_widgets_array);
            }
            ...
            $theme_mods_file = get_template_directory() . '/inc/demo/rental_mods.json';
            if (file_exists($theme_mods_file)) {
                $encode_theme_mods = $wp_filesystem->get_contents($theme_mods_file);
                $import_theme_mods = json_decode($encode_theme_mods, true);
                foreach ($import_theme_mods as $key => $value) {
...
if (!function_exists('stm_generate_title_from_slugs')) {
    function stm_generate_title_from_slugs($post_id, $show_labels = false)
    {
        ...
        if(get_post_type($post_id) == 'listings') {
			if (!empty($title_from) and stm_is_listing() || stm_is_car_dealer()) {
...
if (!function_exists('stm_filter_post_limits')) {
    function stm_filter_post_limits($restrictions, $user_id, $type)
    {
        ...
        if (!empty($user_id)) {
            ...
            $post_status = (!empty($type)) ? 'publish' : 'any';
        ...
        if ($site_color_style != 'site_style_default') {
            ...
            if ($layout == 'motorcycle') {
                ...
                if ($layout !== 'boats') {
                    ...
                    $base_color = get_theme_mod('site_style_base_color', '#b41816 ');
...
if (!function_exists('stm_listings_filter_classified_title')) {
    ...
    if (stm_is_listing() or stm_is_motorcycle()) {
...
function updateListingsStatus ($subs, $old_status, $new_status) {
	if($new_status == 'active') {
		...
		for($q=0;$q<$post_limit;$q++) {
			wp_update_post(array('ID' => $posts[$q]->ID, 'post_status' => 'publish'));
		}
	}
}
