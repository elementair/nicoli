...
$filters = array(
	'reviews' => esc_html__('Opiniones', 'motors'),
	'date' => esc_html__('Fecha', 'motors'),
	'cars' => esc_html__('Número de coches', 'motors'),
	'watches' => esc_html__('Popularidad', 'motors')
);
		...
		<div role="tabpanel" class="tab-pane fade in active" id="stm_all_listing_tab">
			<form action="<?php echo esc_url(stm_get_dealer_list_page()); ?>" method="GET">
				...
				<div class="stm-filter-tab-selects">
					<div class="row">
						<?php if(!empty($stm_filter_dealers_by)): ?>
								...
								<div class="col-md-4 col-sm-6 col-xs-12 stm-select-col">
									<div class="stm-ajax-reloadable">
										<select
											...
											<option value=""><?php esc_html_e('Escoger', 'motors'); echo esc_attr(' ' . stm_get_name_by_slug($stm_filter_dealers)); ?></option>
											<?php
												if ( ! empty( $terms ) ) {
							...
							<div class="stm-location-search-unit">
								<input
									type="text"
                                     ...
                                     placeholder="<?php esc_html_e('Introduce una ubicación', 'motors'); ?>"
                                     autocomplete="off">
		...
		<?php
			if(!empty($user_list)) { ?>
				...
				<?php if(!empty($response['button']) and $response['button'] == 'show'): ?>
					<a class="stm-load-more-dealers button" href="#" data-offset="12"><span><?php esc_html_e('Ordenar por', 'motors') ?></span></a>
				<?php endif; ?>
			<?php } else { ?>
				<h4><?php esc_html_e('No hay distribuidores en sus parámetros de búsqueda', 'motors'); ?></h4>
			<?php }
		?>
	</div>
