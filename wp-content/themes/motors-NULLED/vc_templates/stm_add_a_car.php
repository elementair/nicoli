...
if ($restricted and !$car_edit): ?>
    ...
    <div class="stm-no-available-adds">
        <h3><?php esc_html_e('Publicaciones disponibles', 'motors'); ?>: <span>0</span></h3>
        <p><?php esc_html_e('Usted terminó el límite de anuncios clasificados gratis. Por favor seleccione una de las siguientes', 'motors'); ?></p>
        <div class="clearfix">
            <?php if (stm_pricing_enabled()): ?>
                ...
                if (!empty($stm_pricing_link)): ?>
                    <a href="<?php echo esc_url($stm_pricing_link); ?>" class="button stm-green">
                        <?php esc_html_e('Plan de actualización', 'motors'); ?>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <?php if ($restrictions['role'] == 'user'): ?>
                    <a href="<?php echo esc_url(add_query_arg(array('become_dealer' => 1), stm_get_author_link(''))); ?>"
                       class="button stm-green"><?php esc_html_e('Conviértete en un distribuidor', 'motors'); ?></a>
                <?php endif; ?>
            <?php endif; ?>
            <?php if(is_user_logged_in()): ?>
                <a href="<?php echo esc_url(stm_get_author_link('')); ?>" class="button stm-green-dk"><?php esc_html_e('Mi inventario', 'motors'); ?></a>
            <?php elseif($login_page): ?>
                <a href="<?php echo esc_url(get_permalink( $login_page )); ?>" class="button stm-green-dk"><?php esc_html_e('Registro', 'motors'); ?></a>
            <?php endif; ?>
        </div>
    </div>
<?php endif; ?>
...
<div class="stm_add_car_form <?php echo esc_attr($stm_edit_car_form); echo esc_attr($css_class); ?>">
    ...
    if ($car_edit) {
        if (!is_user_logged_in()) {
            echo '<h4>' . esc_html__('Por favor Iniciar sesión.', 'motors') . '</h4></div>';
            return false;
        }
        ...
        if (!empty($_GET['item_id'])) {
            ...
            if (intval($user_id) != intval($car_user)) {
                echo '<h4>' . esc_html__('Usted no es el dueño de este coche.', 'motors') . '</h4></div>';
                return false;
            }
        } else {
            echo '<h4>' . esc_html__('No hay coche para editar.', 'motors') . '</h4></div>';
            return false;
        }
    } ?>
