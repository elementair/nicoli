    ...
    <div class="col-md-3 col-sm-12 sidebar-sm-mg-bt <?php echo esc_attr($sidebar_pos_classes); ?>">
		...
		if (!empty($modern_filter)){ $counter = 0;
			foreach ($modern_filter as $modern_filter_unit) {
				...
				if (($modern_filter_unit['slider'] == '' || $modern_filter_unit['slider'] == 0) && $modern_filter_unit['slug'] != 'price') { /*<!--If its not price-->*/
										...
										<div class="stm-single-unit-wrapper">
															...
															<?php if (!empty($category_image)){ ?>
																<span class="image">
																	<img class="img-reponsive" src="<?php echo esc_url($category_image); ?>"
																		 alt="<?php esc_html_e('Marca', 'motors'); ?>"/>
																</span>
															<?php } ?>
															<input type="checkbox"
																   name="<?php echo esc_attr($term->slug . '-' . $term->term_id); ?>"
											...
											if ($number_of_images < count($terms)){ ?>
												<div class="stm-modern-view-others">
													<a href=""><?php echo esc_html_e('Ver todo', 'motors'); ?></a>
												</div>
    ...
    <div class="col-md-9 col-sm-12 <?php echo esc_attr($content_pos_classes); ?>">
        <div class="stm-car-listing-sort-units stm-modern-filter-actions clearfix">
            <div class="stm-modern-filter-found-cars">
                <h4><span
                        class="orange"><?php echo esc_attr($listings->found_posts); ?></span> <?php esc_html_e('Vehículos disponibles', 'motors'); ?>
                </h4>
            </div>
            <?php
            $view_list = '';
            ...
            <div class="stm-sort-by-options clearfix">
                <span><?php esc_html_e('Ordenar por:', 'motors'); ?></span>
                <div class="stm-select-sorting">
                    <select>
                        <option value="date_high" selected><?php esc_html_e('Fecha: más reciente primero', 'motors'); ?></option>
                        <option value="date_low"><?php esc_html_e('Fecha: más antigua primero', 'motors'); ?></option>
                        <option value="price_low"><?php esc_html_e('Precio: más bajo primero', 'motors'); ?></option>
                        <option value="price_high"><?php esc_html_e('Precio: más caros primero', 'motors'); ?></option>
                        <option value="mileage_low"><?php esc_html_e('Kilometraje: el más bajo primero', 'motors'); ?></option>
                        <option value="mileage_high"><?php esc_html_e('Kilometraje: el más alto primero', 'motors'); ?></option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <?php if ($listings->have_posts()): ?>
            ...
            <div class="stm-isotope-sorting">
                ...
                <a class="button stm-show-all-modern-filter stm-hidden-filter"><?php esc_html_e('Mostrar todo', 'motors'); ?></a>
