...
if(!stm_is_boats() and !stm_is_motorcycle()):
	...
	$required_fields = array(
		'make' => __('Hacer', 'motors'),
		'model' => __('Modelo', 'motors'),
		'first_name' => __('Detalles del usuario <br/> Nombre', 'motors'),
		'last_name' => __('Apellido', 'motors'),
	);

	$non_required_fields = array(
		'transmission' => __('Transmisión', 'motors'),
		'mileage' => __('Kilometraje', 'motors'),
		'vin' => __('Vin', 'motors'),
		'exterior_color' => __('Color exterior', 'motors'),
		'interior_color' => __('Color interior', 'motors'),
		'owner' => __('Propietaria', 'motors'),
		'exterior_condition' => __('Condicion exterior', 'motors'),
		'interior_condition' => __('Condicion interior', 'motors'),
		'accident' => __('Accidente', 'motors'),
		'stm_year' => __('Año', 'motors'),
		'video_url' => __('URL del vídeo', 'motors'),
		'comments' => __('Comentarios', 'motors')
	);

	if(is_singular(stm_listings_post_type())) {
		$body = sprintf(__('Solicitud de %s', 'motors'), get_the_title());
	} else {
		$body = '';
	}
	$mail_send = false;
	...
	foreach($required_fields as $key => $field) {
		...
		if(!empty($_POST[$key])) {
			...
			$errors[$key] = __('Porfavor llene', 'motors') . ' ' . $field . ' ' . __('field', 'motors') . '<br/>';
		}
	...
	if(!empty($_POST['email']) and is_email($_POST['email'])) {
		...
		$errors['email'] = __('Tu dirección de correo electrónico no es válida', 'motors') . '<br/>';
	}

	// Check phone
	if(!empty($_POST['phone']) and is_numeric($_POST['phone'])) {
		$body .= __('Teléfono', 'motors') . ' - ' . intval($_POST['phone']) . '<br/>';
	} else {
		$errors['phone'] = __('Tu teléfono es invalido', 'motors') . '<br/>';
	}
	...
	if( ! empty( $_FILES ) ) {
		$body .= __('Imágenes subidas', 'motors') .':<br/>';
	...
	if(!empty($body) and empty($errors)) {
		...
		if(is_singular(stm_listings_post_type())) {
			$subject = esc_html__( 'Imágenes subidas', 'motors' );
		} else {
			$subject = esc_html__( 'Vender una solicitud de coche', 'motors' );
		}
			...
			<div class="row">
				<div class="col-md-4 col-sm-4">
					<a href="#step-one" class="form-navigation-unit active" data-tab="step-one">
						...
						<div class="title heading-font"><?php esc_html_e('Información del coche', 'motors'); ?></div>
						<div class="sub-title"><?php esc_html_e('Añada los detalles de su vehículo', 'motors'); ?></div>
					</a>
				</div>
				<div class="col-md-4 col-sm-4">
					<a href="#step-two" class="form-navigation-unit" data-tab="step-two">
						<div class="number heading-font">2.</div>
						<div class="title heading-font"><?php esc_html_e('Condición del vehículo', 'motors'); ?></div>
						<div class="sub-title"><?php esc_html_e('Añada los detalles de su vehículo', 'motors'); ?></div>
					</a>
				</div>
				<div class="col-md-4 col-sm-4">
					<a href="#step-three" class="form-navigation-unit" data-tab="step-three">
						...
						<div class="title heading-font"><?php esc_html_e('Detalles de contacto', 'motors'); ?></div>
						<div class="sub-title"><?php esc_html_e('Tus detalles de contacto', 'motors'); ?></div>
					</a>
				</div>
			</div>
		</div>
			...
			<form method="POST" action="#error-fields" enctype="multipart/form-data">
				...
				<div class="form-content-unit active" id="step-one">
							...
							<div class="form-upload-files">
								<div class="clearfix">
									<div class="stm-unit-photos">
										<h5 class="stm-label-type-2"><?php esc_html_e('Sube tus fotos del coche', 'motors'); ?></h5>
										<div class="upload-photos">
											<div class="stm-pseudo-file-input">
												<div class="stm-filename"><?php esc_html_e('Elija el archivo...', 'motors'); ?></div>
									...
									<div class="stm-unit-url">
										<h5 class="stm-label-type-2">
											<?php esc_html_e('Proporcionar el url de un video alojado de su coche', 'motors'); ?>
										</h5>
					...
					<a href="#" class="button sell-a-car-proceed" data-step="2">
						<?php esc_html_e('Guardar y continuar', 'motors'); ?>
					</a>
				</div>
				...
				<div class="form-content-unit" id="step-two">
					<div class="vehicle-condition">
						<div class="vehicle-condition-unit">
							...
							<div class="title h5"><?php esc_html_e('¿Cuál es la condición exterior?', 'motors'); ?></div>
							<label>
								<input type="radio" name="exterior_condition" value="<?php esc_html_e('Extra limpio', 'motors'); ?>" checked/>
								<?php esc_html_e('Extra clean', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="exterior_condition" value="<?php esc_html_e('Limpiar', 'motors'); ?>"/>
								<?php esc_html_e('Limpiar', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="exterior_condition" value="<?php esc_html_e('Promedio', 'motors'); ?>"/>
								<?php esc_html_e('Promedio', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="exterior_condition" value="<?php esc_html_e('Debajo del promedio', 'motors'); ?>"/>
								<?php esc_html_e('Debajo del promedio', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="exterior_condition" value="<?php esc_html_e('I don\'t know', 'motors'); ?>"/>
								<?php esc_html_e('No lo sé', 'motors'); ?>
							</label>
						</div>
						<div class="vehicle-condition-unit">
							<div class="icon buoy"><i class="stm-icon-buoy"></i></div>
							<div class="title h5"><?php esc_html_e('¿Cuál es la condición interior?', 'motors'); ?></div>
							<label>
								<input type="radio" name="interior_condition" value="<?php esc_html_e('Extra limpio', 'motors'); ?>" checked/>
								<?php esc_html_e('Extra limpio', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="interior_condition" value="<?php esc_html_e('Limpiar', 'motors'); ?>"/>
								<?php esc_html_e('Limpiar', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="interior_condition" value="<?php esc_html_e('Promedio', 'motors'); ?>"/>
								<?php esc_html_e('Promedio', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="interior_condition" value="<?php esc_html_e('Debajo del promedio', 'motors'); ?>"/>
								<?php esc_html_e('Debajo del promedio', 'motors'); ?>
							</label>
							<label>
								<input type="radio" name="interior_condition" value="<?php esc_html_e('No lo sé', 'motors'); ?>"/>
								<?php esc_html_e('No lo sé', 'motors'); ?>
							</label>
						</div>
						<div class="vehicle-condition-unit">
							<div class="icon buoy-2"><i class="stm-icon-buoy-2"></i></div>
							<div class="title h5"><?php esc_html_e('Ha estado el vehículo en accidente', 'motors'); ?></div>
							<label>
								<input type="radio" name="accident" value="<?php esc_html_e('Si', 'motors'); ?>"/>
							...
							<label>
								<input type="radio" name="accident" value="<?php esc_html_e('No lo sé', 'motors'); ?>"/>
								<?php esc_html_e('No lo sé', 'motors'); ?>
							</label>
						</div>
					</div>
				...
				<div class="form-content-unit" id="step-three">
					...
					<div class="clearfix">
						<div class="pull-left">
							<input type="submit" value="<?php esc_html_e('Guardar y terminar', 'motors'); ?>" />
						</div>
						<div class="disclaimer">
							<?php esc_html_e(' Al enviar este formulario, solicitará un valor de intercambio gratuito y
será contactado dentro de las 48 horas por un representante de ventas.', 'motors'); ?>
						</div>
					</div>
				</div>
			</form>
		...
		(function($) {
		    ...
		    $(document).ready(function() {
				...
				$('.stm-plus').click(function(e){
					...
					if(i < 5) {
						i++;
						$('.upload-photos').append('<div class="stm-pseudo-file-input generated"><div class="stm-filename"><?php esc_html_e('Elija el archivo...', 'motors'); ?></div><div class="stm-plus"></div><input class="stm-file-realfield" type="file" name="gallery_images_' + i + '"/></div>');
					}
				})
