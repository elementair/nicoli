...
if(empty($search_label)) {
	$search_label = esc_html__('Buscar inventario', 'motors');
}
			...
			<div class="tab-content">
					...
					<?php foreach($filter_cats as $filter_cat): $filter_cats_counter++; ?>
						<?php if(!empty($filter_cat[0]) and !(empty($filter_cat[1]))): ?>
							...
							<div role="tabpanel" class="tab-pane <?php if($filter_cats_counter == 1) {echo esc_attr('active');} ?>" id="car-listing-category-<?php echo esc_attr($filter_cat[0]); ?>">
								<div class="found-cars-clone"><div class="found-cars heading-font"><i class="stm-icon-car"></i><?php esc_html_e('disponible','motors'); ?> <span class="blue-lt"><?php echo esc_attr($listing_cars->found_posts); ?> <?php echo esc_html($atts["found_cars_prefix"]); ?></span></div></div>
											...
											<div class="col-xs-12 text-center">
												<div class="dp-in">
													<a class="load-more-btn" href="<?php echo esc_url(stm_get_listing_archive_link()).'?'.esc_attr($filter_cat[1]).'='.esc_attr($filter_cat[0]); ?>">
														<?php esc_html_e('Mostrar todo', 'motors'); ?>
													</a>
												</div>
											</div>
										</div>
				...
				<?php if(!empty($enable_search) and $enable_search): ?>
					<div role="tabpanel" class="tab-pane <?php if($filter_cats_counter == 0) {echo esc_attr('active');} ?>" id="car-listing-tab-search">
						<div class="found-cars-clone"><div class="found-cars heading-font"><i class="stm-icon-car"></i><?php esc_html_e('disponible','motors'); ?> <span class="blue-lt"><?php echo esc_attr($all_cars->found_posts); ?> <?php esc_html_e('coches','motors'); ?></span></div></div>
												...
												<div class="col-md-<?php echo esc_attr($filter_columns_number); ?> col-sm-6">
													<div class="form-group">
														<select name="<?php echo ($taxonomy_term_key == "price") ? "max_price" : esc_attr($taxonomy_term_key) ?>" class="form-control">
															<option value=""><?php echo esc_html__('Seleccionar', 'motors').' '.esc_html__(stm_get_name_by_slug($taxonomy_term_key), 'motors'); ?></option>
										...
										<div class="col-md-3 col-sm-6">
											<div class="row">
												<div class="col-md-8 col-sm-12">
													<button type="submit" class="button icon-button"><i class="stm-icon-search"></i><?php esc_html_e('Buscar', 'motors'); ?></button>
												</div>
												<div class="col-md-4 hidden-sm hidden-xs">
													<a href="" class="reset-all reset-styled" title="<?php esc_html_e('Restablecer campos de búsqueda', 'motors'); ?>"><i class="stm-icon-reset"></i></a>
												</div>
											</div>
										</div>
									</form>
