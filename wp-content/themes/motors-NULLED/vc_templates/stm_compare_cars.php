...
$add_to_text = esc_html__('Añadir coche para comparar', 'motors');

if(stm_is_boats()) {
    $add_to_text = esc_html__('Añadir barco para comparar', 'motors');
} elseif(stm_is_motorcycle()) {
    $add_to_text = esc_html__('Añadir vehículo para comparar', 'motors');
}

$title_text = esc_html__('Comparar vehiculos', 'motors');
if(stm_is_boats()) {
    $title_text = esc_html__('Comparar barcos', 'motors');
}
                    ...
                    <div
                                ...
                                <div class="image">
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <img class="stm-compare-empty"
                                             ...
                                             alt="<?php esc_html_e('Vacío', 'motors'); ?>"/>
                                    <?php }; ?>
                                </div>
                            </div>
                        </a>
                        <div class="remove-compare-unlinkable">
							<span
								...
								<span><?php esc_html_e('Quitar de la lista', 'motors'); ?></span>
							</span>
                        </div>
                        <a href="<?php the_permalink(); ?>" class="rmv_txt_drctn">
                        ...
                        <span class="btn btn-default add-to-compare hidden" data-action="remove"
							...
							<?php esc_html_e('Eliminar comparacióm', 'motors'); ?>
						</span>
                    </div> <!--md-3-->
                <?php endwhile; ?>
                    ...
                    <div class="col-md-3 col-sm-3 col-xs-4 compare-col-stm-empty">
                        <a href="<?php echo esc_url(stm_get_listing_archive_link()); ?>">
                            <div class="image">
                                ...
                                <img class="stm-compare-empty"
                                     src="<?php echo esc_url(get_stylesheet_directory_uri() . '/assets/images/compare-empty.jpg'); ?>"
                                     alt="<?php esc_html_e('Vacio', 'motors'); ?>"/>
                            </div>
                        ...
                        <?php if (!empty($filter_options)): ?>
                            <div class="compare-values">
                                ...
                                <table>
									...
									<tr>
										<td class="compare-value-hover">
											<div class="h5" data-option="<?php esc_html_e('Precio', 'motors'); ?>">&nbsp;
												<?php $price = get_post_meta(get_the_id(), 'price', true); ?>
												<?php $sale_price = get_post_meta(get_the_id(), 'sale_price', true); ?>
												<?php $car_price_form_label = get_post_meta(get_the_ID(), 'car_price_form_label', true); ?>
												<?php if (empty($car_price_form_label)): ?>
    ...
    <?php if (!empty($compares)): ?>
        <?php if ($compares->have_posts()): ?>
            <div class="row row-4 row-compare-features hidden-xs">
                <div class="col-md-3 col-sm-3">
                    <h4 class="stm-compare-features"><?php esc_html_e('Características adicionales', 'motors'); ?></h4>
                </div>
                <?php while ($compares->have_posts()): $compares->the_post(); ?>
