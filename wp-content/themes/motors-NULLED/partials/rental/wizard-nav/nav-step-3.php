<?php
    ...
    if(!empty($billing['total'])) {
        $payment = sprintf(__('Total Estimado - %s', 'motors') , $billing['total']);
    }
...
<div class="inner <?php echo esc_attr($class); ?>">
    <a href="<?php echo esc_url(stm_woo_shop_checkout_url()); ?>" class="top heading-font">
        ...
        <label><?php esc_html_e('Reserve su vehículo', 'motors'); ?></label>
    </a>
    <div class="content">
        <div class="first">
            <h5><?php esc_html_e('Tu información', 'motors'); ?></h5>
            <div><?php echo sanitize_text_field($info); ?></div>
        </div>
        <div class="second">
            <h5><?php esc_html_e('Información del pago', 'motors'); ?></h5>
            <div>
